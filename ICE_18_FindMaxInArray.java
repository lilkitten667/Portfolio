...
class MaxArray extends Object
{
    ...
    public void FindMaxInArray(int[] ah )
    {
        int max = ah[0];
        int locationOfMax;
        for (locationOfMax = 0; locationOfMax < ah.length; locationOfMax++)
        {
            if (ah[locationOfMax] > max)
            {
                max = ah[locationOfMax];
                return;
            }
        }
